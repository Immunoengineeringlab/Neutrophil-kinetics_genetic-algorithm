function [Chi_sq] = Error_file(parameters)
...
% inter1 is the zero vector of size [1Xn] 
% inter2 is the zero vector of size [1Xh]
% both inter1 and inter2 are used to initialize the initial condition vector in line 41 below. This allows us to change the number of differentially equations
% automatically when the error function code is called by genetic algorithm for different parameter sets.
...
mature=y(:,2+n:2+n+h); % solution of the ODE at different maturation compartments
y2=sum(mature,2); %sum the solution values in all maturation compartments making up the maturation pool
y3=y(:,3+n+h); % percentage neutrophils in blood 
y4=y(:,4+n+h); % percentage neutrophils in peritoneum


%calculating Chi square__________________________________________________________

p1=interp1(t,y2,tpts); % interpolating the solutions to get the values at experiment timepoints. Here linear interpolation is used.
p2=interp1(t,y3,tpts);
p3=interp1(t,y4,tpts);

Chi_sq=sum( ((p1-tptvals(1,:))./standard(1,:)).^2     +   ((p2-tptvals(2,:))./standard(2,:)).^2   +     ((p3-tptvals(3,:))./standard(3,:)).^2); % chi square estimation
